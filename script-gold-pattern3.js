...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --------------------------------------------------
    // PAGE-BG — スクロールでガラッと変わる（黒→ダーク→ライト→ダーク→ゴールド→…）
    // --------------------------------------------------
    const pageBg = document.getElementById('page-bg');

    // パターン2の温かみを40%取り入れ（クリーム・ゴールドティント）
    const BG_COLORS = {
        dark:    '#120f0c',   /* ほんのりウォームなダーク */
        darkalt: '#141210',
        light:   '#fef9e7',   /* パターン2のクリーム系 */
        gold:    '#F7C02D',
        black:   '#0a0a0a',
    };

    // セクションごとに背景が劇的に変化（ゴールド多め）
    const sectionBgMap = [
    ...
    function transitionBg(newBg, sectionEl) {
        ...
        // セクションのdata-section-themeに合わせてヘッダーを切り替え（黒背景は常にdark）
        let theme = sectionEl ? sectionEl.dataset.sectionTheme : 'dark';
        if (newBg === BG_COLORS.black) theme = 'dark';
    ...
    // 初期はファーストビュー（黒背景 → ヘッダーはダーク）
    if (pageBg) {
        pageBg.style.backgroundColor = BG_COLORS.black;
    }
